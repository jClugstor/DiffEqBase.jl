...
* `alias`: an `AbstractAliasSpecifier` object that holds fields specifying which variables to alias
  ...
  associated with that problem type. The alias specifier types follow the naming convention `ProbAliasSpecifier`, where "Prob" 
  is the first part of the problem type name, e.g. the alias specifier for a LinearProblem is `LinearAliasSpecifier`.
  Can also be a `Bool`, set to `true` to alias every variable possible, or to `false` to disable aliasing.
